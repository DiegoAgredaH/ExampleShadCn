...
Run the `tintta-widgets` init command to setup your project:

```bash
npx tintta-widgets@latest init
```
...
When running `npx tintta-widgets@latest init`, your tailwind config for content will be overwritten. To fix this, change the `content` section with the code below to your `tailwind.config.cjs` file:
...
```bash
npx tintta-widgets@latest add button
```
